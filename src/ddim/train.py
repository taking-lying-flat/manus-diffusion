...
def create_diffusion(model, diffusion_steps, device, beta_schedule="linear", sampling_steps=50, eta=0.0):
...
def train(config_path="config.yaml", resume=None):
    ...
    sample_batch_size = dataset_config['sample_batch_size']
    attention_resolutions = dataset_config['attention_resolutions']
    
    save_dir = os.path.join(config['logging']['save_dir'], dataset_name)
    os.makedirs(save_dir, exist_ok=True)
    logger = setup_logger(log_dir=save_dir)
    ...
    diffusion = create_diffusion(
        ...
        sampling_steps=config['diffusion'].get('sampling_steps', 50),
        eta=config['diffusion'].get('eta', 0.0),
    )
    ...
    if dataset_name == 'cifar10':
        ...
        dataset = torchvision.datasets.STL10(
            root=config['data']['data_dir'], split='unlabeled', download=True, transform=transform
        )
    else:
    ...
    logger.info("=" * 60)
    logger.info("🚀 DDIM Training Configuration")
    logger.info("=" * 60)
    ...
    logger.info(f"🎲 Sampling steps: {config['diffusion'].get('sampling_steps', 50)}")
    logger.info(f"🎲 Sampling eta: {config['diffusion'].get('eta', 0.0)}")
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        if (epoch + 1) % config['logging']['sample_interval'] == 0:
            ...
            save_image(samples_vis, os.path.join(save_dir, f"samples_epoch_{epoch + 1}.png"), nrow=8)
            logger.info(f"🖼️  Saved samples to samples_epoch_{epoch + 1}.png")

            detailed_metrics = visualize_denoising(
                diffusion, vis_batch, t_vis, save_path=os.path.join(save_dir, f"denoise_epoch_{epoch + 1}.png")
            )
